...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., rpe=True, scaling_alpha=1, w_init = "normal"):
        ...
        self.qkv = MergedLinear(in_features=dim, out_features=dim*3, \
                                r = 8, bias=qkv_bias, enable_lora=[True, False, True], \
                                merge_weights=False)  # Changed
    ...
    def forward(self, x, mask=None):
        ...
        # q, k, v = self.q(x).reshape(B_, self.num_heads, N, C//self.num_heads),\
        #           self.k(x).reshape(B_, self.num_heads, N, C//self.num_heads),\
        #           self.v(x).reshape(B_, self.num_heads, N, C//self.num_heads)  # make torchscript happy (cannot use tensor as tuple)
        q = q * self.scale
...
class SwinTransformerLoRa(nn.Module):
    r""" Swin Transformer
