...
class Token_performer(nn.Module):
    ...
    def prm_exp(self, x):
        ...
        wtx = torch.einsum('bti,mi->btm', x, self.w)  #changed
    ...
    def single_attn(self, x):
        ...
        kptv = torch.einsum('bin,bim->bnm', v, kp)  # (B, emb, m)  #changed
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, type_adapter = "series", param_ratio=2):
        super().__init__()
            ...
            if "ffn" in self.type_adapter.lower():
                ...
                if "parallel" in self.type_adapter.lower():
                    ...
                    self.parallel_mlp1 = ParallelMlp(in_features=self.dim,
                                                     hidden_features=mlp_hidden_dim // self.param_ratio,
                                                     act_layer=act_layer, drop=drop)
                else:
                    self.parallel_mlp1 = ParallelMlp(in_features=self.dim,
                                                     hidden_features=mlp_hidden_dim // self.param_ratio,
...
class T2T_ViT_Adapters(nn.Module):
    def __init__(self,
                 img_size=224,
                 ...
                 sample_size=32,
                 use_abs=False,
                 type_adapter="series", param_ratio=3
                 ):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([
            Block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                  drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, type_adapter = self.type_adapter, param_ratio=self.param_ratio)
...
def T2t_vit_14_Adapters(
    ...
    model = T2T_ViT_Adapters(
        ...
        sample_size=sample_size,
        use_abs=use_abs,
        type_adapter=type_adapter, param_ratio=param_ratio
    )
    return model