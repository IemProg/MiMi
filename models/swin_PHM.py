...
"hypercomplex_adapters": True, "adapter_config_name": "adapter", "hypercomplex_division": 12,
...
class HyperComplexAdapter(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., config_PHM = PHM_params, reduction_factor = 32):
        super().__init__()
        self.activation = act_layer()
        self.input_dim = in_features
        self.out_features = out_features or in_features
        self.hidden_features = (hidden_features or in_features) // reduction_factor

        self.down_sampler = PHMLinear(in_features=self.input_dim,
                                      out_features=self.hidden_features,
                                      bias=True,
                                      c_init=config_PHM["phm_c_init"],
                                      phm_dim=config_PHM["hypercomplex_division"],
                                      learn_phm=config_PHM["learn_phm"],
                                      w_init=config_PHM["hypercomplex_nonlinearity"],
                                      shared_phm_rule=config_PHM["shared_phm_rule"],
                                      factorized_phm=config_PHM["factorized_phm"],
                                      shared_W_phm=config_PHM["shared_W_phm"],
                                      factorized_phm_rule=config_PHM["factorized_phm_rule"],
                                      phm_rank=config_PHM["phm_rank"],
                                      phm_init_range=config_PHM["phm_init_range"],
                                      kronecker_prod=config_PHM["kronecker_prod"])
        self.up_sampler = PHMLinear(in_features=self.hidden_features,
                                    ...
                                    kronecker_prod=config_PHM["kronecker_prod"])
    def forward(self, x):
        #TODO: It is not really the same architecture as Houlsby paper, need a residual connection
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, rpe=True, type_adapter="series", param_ratio=3):
        super().__init__()
        self.dim = dim
        ...
        # Parallel Adapters
        self.parallel_mlp1 = HyperComplexAdapter(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop, reduction_factor = param_ratio)
        self.parallel_mlp2 = HyperComplexAdapter(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop, reduction_factor = param_ratio)

        if self.type_adapter == "parallel":
    ...
    def forward(self, x):
        ...
        # FFN
        x = shortcut + self.drop_path(self.parallel_mlp1(x))
        x_top = self.drop_path(self.mlp(self.norm2(x)))

        if self.type_adapter == "series":
            x_feed = self.drop_path(self.parallel_mlp2(x_top))
        else:
            x_feed = self.drop_path(self.parallel_mlp2(self.norm3(x))) + x_top
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 rpe=True, type_adapter="series", ratio_param=3):
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                 ...
                                 rpe=rpe, type_adapter=type_adapter, param_ratio=ratio_param)
...
class SwinTransformerPHM(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               ...
                               rpe=rpe, type_adapter=type_adapters, ratio_param=ratio_param)
