...
class WindowAttention(nn.Module):
    ...
    def forward_with_attention(self, x, mask=None, policy= None):
        """
        Args:
            x: input features with shape of (num_windows*B, N, C)
            mask: (0/-inf) mask with shape of (num_windows, Wh*Ww, Wh*Ww) or None
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, policy=None):
        ...
        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows, mask=self.attn_mask, policy=policy)  # nW*B, window_size*window_size, C
    ...
    def forward_with_attention(self, x, policy=None):
        H, W = self.input_resolution
        B, L, C = x.shape
        assert L == H * W, "input feature has wrong size"
        ...
        # W-MSA/SW-MSA
        attn_windows, attn = self.attn.forward_with_attention(x_windows, mask=self.attn_mask, policy=policy)  # nW*B, window_size*window_size, C
...
class SwinTransformerPrunning(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 pruning_loc=None, token_ratio=None,
                 distill=False, viz_mode=False,
                 **kwargs):
        super().__init__()
        ...
        if self.use_multiscale:
            ...
            for i, blk in enumerate(self.layers):
                if i in self.pruning_loc:
                    ...
                    if self.training:
                        ...
                        cls_policy = torch.ones(B, 1, 1, dtype=hard_keep_decision.dtype,
                    ...
                    if self.training:
                        ...
                        x = blk(x)
                all_layers.append(x)
            return all_layers, prev_decision.detach(), out_pred_prob
        else:
            p_count = 0
            out_pred_prob = []
            init_n = 96 #14 * 14
            ...
            for i, blk in enumerate(self.layers):
                if i in self.pruning_loc:
                    ...
                    spatial_x = x[:, 1:]
                    print("\t\t spatial_x: ", spatial_x.shape)
                    print("\t\t prev_decision: ", prev_decision.shape)
                    pred_score = self.score_predictor[p_count](spatial_x, prev_decision).reshape(B, -1, 2)
                    if self.training:
                        hard_keep_decision = F.gumbel_softmax(pred_score, hard=True)[:, :, 0:1] * prev_decision
                        out_pred_prob.append(hard_keep_decision.reshape(B, init_n))
                    ...
                    if self.training:
                        ...
                        x = blk(x)
            return [x], prev_decision, out_pred_prob

    def forward(self, x):
        xs, prev_decision, out_pred_prob = self.forward_features(x)
        x_last = self.norm(xs[-1])  # [B, L, C]

        #Need to check this line below
        features = xs[:, 1:]
        ...
        if self.training:
            if self.distill:
                return outs, features, prev_decision.detach(), out_pred_prob
            else:
                return outs, out_pred_prob
        else:
            return outs
