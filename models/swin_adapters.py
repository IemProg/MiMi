...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, rpe=True, type_adapter="parallel_ffn", param_ratio=32):
        super().__init__()
        self.dim = dim
            ...
            if "ffn" in self.type_adapter.lower():
                if "parallel" in self.type_adapter.lower():
                    ...
                    self.parallel_mlp2 = ParallelMlp(in_features=self.dim,
                                                     ...
                                                     act_layer=act_layer2, drop=drop, type="parallel", mlp_bias=False)
                else:
                    self.parallel_mlp2 = ParallelMlp(in_features=self.dim,
                                                     hidden_features=mlp_hidden_dim // param_ratio,
                                                     act_layer=act_layer2, drop=drop, mlp_bias=False)
                ...
                if self.type_adapter.lower() == "parallel":
                    ...
                    self.parallel_mlp1 = ParallelMlp(in_features=self.dim,
                                                     hidden_features=mlp_hidden_dim // param_ratio,
                                                     act_layer=act_layer2, drop=drop, type="series", mlp_bias=False)
                    self.parallel_mlp2 = ParallelMlp(in_features=self.dim,
                                                     hidden_features=mlp_hidden_dim // param_ratio,
                                                     act_layer=act_layer2, drop=drop, type="series", mlp_bias=False)
    ...
    def forward(self, x):
        ...
        if self.type_adapter == "series":
            ...
            x_attn = x + self.norm3(self.parallel_mlp1(shortcut))
            ...
            x = x + self.norm3(self.parallel_mlp1(shortcut))
            x = x + self.drop_path(self.mlp(self.norm2(x)))
        elif self.type_adapter == "scaled_adapters":
            x_attn = x + self.adapt_scale * self.parallel_mlp1(shortcut)
            x = self.drop_path(self.mlp(self.norm2(x_attn))) + self.adapt_scale * self.parallel_mlp2(x_attn)
        else:
    ...
    def flops(self):
        ...
        # series-attn
        flops += 2 * H * W * self.dim * (int(self.dim * self.mlp_ratio) // self.param_ratio)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 rpe=True, type_adapter="series", ratio_param=32):
...
class SwinTransformerWithAdapters(nn.Module):
    r""" Swin Transformer
