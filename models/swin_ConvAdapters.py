...
class ParallelMlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., kernel = 3, upsampling=True):
        super().__init__()
        ...
        self.conv1 = nn.Conv2d(self.in_features, self.hidden_features, kernel_size=self.kernel)
        self.act = act_layer()
        self.deconv1 = nn.ConvTranspose2d(self.hidden_features, self.out_features, kernel_size = self.kernel)
        self.drop = nn.Dropout(drop)

        self.bn1 = nn.BatchNorm2d(self.hidden_features)
        self.bn2 = nn.BatchNorm2d(self.out_features)

        # TODO
        self.fc1 = nn.Conv2d(self.in_features, self.hidden_features, kernel_size=self.kernel)
        self.fc2 = nn.Conv2d(self.hidden_features, self.out_features, kernel_size=self.kernel)
    def forward(self, x):
        if self.upsampling:
            size_x = x.size()
            x = self.conv1(x)
            ...
            x = self.deconv1(x, output_size=size_x)
            x = self.act(x)
            x = self.bn2(x)
            x = self.drop(x)
        else:
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        # Parallel Adapters
        self.parallel_mlp = ParallelMlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
    ...
    def forward(self, x):
        ...
        x_copy = x.view(B, C, H, W) # Used for convolution 1x1
        x = x.view(B, H * W, C)

        # FFN
        x = shortcut + self.drop_path(x)
        #x_right = x + self.drop_path(self.mlp(self.norm2(x)))
        x_right = self.drop_path(self.mlp(self.norm2(x)))

        # FIX: DONE
        #x_copy = x_copy + self.drop_path(x_copy)
        x_copy = self.drop_path(x_copy)
        #x_left = x_copy + self.drop_path(self.parallel_mlp(self.norm3(x_copy)))
        # TODO: Norm3 is included in parallel MLP
        x_left = x_copy + self.drop_path(self.parallel_mlp(x_copy))
...
class SwinTransformerWithConvAdapters(nn.Module):
    r""" Swin Transformer
