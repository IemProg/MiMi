...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., rpe=True, scaling_alpha=1, w_init = "LoRa"):
        ...
        self.phm_dim = 4
        self._in_feats_per_axis = in_features // self.phm_dim
        self._out_feats_per_axis = out_features // self.phm_dim
        self.phm_rank = 1

        self.phm_init_range = 0.01
        self.phm_rule = nn.Parameter(torch.FloatTensor(self.phm_dim, self.phm_dim, self.phm_dim), requires_grad=True)

        self.W_left1 = nn.Parameter(torch.Tensor(size=(self.phm_dim, self._in_feats_per_axis, self.phm_rank)), requires_grad=True)
        self.W_right1 = nn.Parameter(torch.Tensor(size=(self.phm_dim, self.phm_rank, self._out_feats_per_axis)), requires_grad=True)
        self.W_left2 = nn.Parameter(torch.Tensor(size=(self.phm_dim, self._in_feats_per_axis, self.phm_rank)), requires_grad=True)
        self.W_right2 = nn.Parameter(torch.Tensor(size=(self.phm_dim, self.phm_rank, self._out_feats_per_axis)), requires_grad=True)
        self.phmb = nn.Parameter(torch.Tensor(out_features), requires_grad=True)
    ...
    def reset_parameters(self):
        nn.init.zeros_(self.phmb.data)
        if self.w_init == "glorot-normal":
            for i in range(self.phm_dim):
                self.phm_rule.data[i] = glorot_normal(self.W.data[i])
            for i in range(self.phm_dim):
            ...
            for i in range(self.phm_dim):
                self.phm_rule.data[i] = glorot_uniform(self.W.data[i])
            for i in range(self.phm_dim):
            ...
            for i in range(self.phm_dim):
                self.phm_rule.data[i].normal_(mean=0, std=self.phm_init_range)
            for i in range(self.phm_dim):
    ...
    def phm_forward(self, x, phm_rule, w):
        return matvec_product(W=w, x=x,  bias=self.phmb, phm_rule=phm_rule, kronecker_prod=False)
    ...
    def forward(self, x, mask=None):
        ...
        W1 = torch.bmm(self.W_left1, self.W_right1) #* self.scaling
        W2 = torch.bmm(self.W_left2, self.W_right2) #* self.scaling

        q_delta = self.phm_forward(lora_input, self.phm_rule, W1) * self.scaling
        v_delta = self.phm_forward(lora_input, self.phm_rule, W2) * self.scaling
...
class SwinTransformerKAdapter(nn.Module):
    r""" Swin Transformer
