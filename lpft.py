...
def main(config):
    ...
    # Get only the head initialization
    model_copy.head.weight = model.module.head.weight
    model_copy.head.bias = model.module.head.bias

    model.module = model_copy
    ...
    if config.MODEL.FINETUNE == 3:
        ...
        # Change it to ImageNet, number of classes
        classifier = nn.Linear(net.num_features, 1000)
        net.head = classifier
        ...
        pretrained_dict = {k: v for k, v in checkpoint["model"].items()}
        net1_dict.update(pretrained_dict)
        msg = net.load_state_dict(net1_dict)

        net.cuda()
        # logger.info(str(model))
        optimizer = build_optimizer(config, net)
        if config.AMP_OPT_LEVEL != "O0":
            model, optimizer = amp.initialize(net, optimizer, opt_level=config.AMP_OPT_LEVEL)

        if not config.DEBUG:
            model = torch.nn.parallel.DistributedDataParallel(net, device_ids=[config.LOCAL_RANK],
                                                              broadcast_buffers=False, find_unused_parameters=True)
            model_without_ddp = model.module
            model_without_ddp.head = copy.deepcopy(model_copy.head)
        else:
            model_without_ddp = model
            model_without_ddp.head = copy.deepcopy(model_copy.head)

        # Change it back to dataset's number of classes
        model.module.head = copy.deepcopy(model_copy.head)

    for epoch in range(config.TRAIN.START_EPOCH, 60):
        epoch = (phase * 100) + epoch
        if not config.DEBUG:
            data_loader_train.sampler.set_epoch(epoch)
    ...
    logger.info(f"\t\t Done with phase 2")
...
if __name__ == "__main__":
    ...
    for dset in datasets[3:]:  #[:1]datasets[0:1]
